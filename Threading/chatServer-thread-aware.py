...
def broadcastMessages(convos):
  # Build a string to send to everyone
  ret = "{} Users\n".format(len(conversations.keys()))
  for p in convos.keys():
    ret += "{}: {}\n".format(convos[p]['name'], convos[p]['message'])
  # send to everyone!
  for clientAddress in convos.keys():
    server.sendto(ret.encode('utf-8'), clientAddress)
  print("Send messages to {} users.".format(len(convos)))
  # Note that since this is UDP, once a user leave, the server has no idea,
  # so it will keep sending to all 'dead' users!
...
if __name__ == "__main__":
    ...
    while True:
        ...
        broadcastMessages(conversations)
